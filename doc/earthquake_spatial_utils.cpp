...
/*
// Example: Creating and using the earthquake map widget
#include "EarthquakeMapWidget.h"
#include "SpatialUtils.h"
#include <QtWidgets/QApplication>
...
class EarthquakeMonitorWindow : public QMainWindow
{
    Q_OBJECT

public:
    EarthquakeMonitorWindow(QWidget *parent = nullptr)
        : QMainWindow(parent)
    {
        setupUI();
        connectSignals();
...
private slots:
    void addRandomEarthquake()
    {
        EarthquakeData eq;
        eq.latitude = (rand() % 180) - 90.0;  // -90 to 90
        eq.longitude = (rand() % 360) - 180.0; // -180 to 180
        eq.magnitude = 2.0 + (rand() % 60) / 10.0; // 2.0 to 8.0
        eq.depth = rand() % 700; // 0 to 700 km
        eq.timestamp = QDateTime::currentDateTime();
        eq.location = QString("Location %1").arg(rand() % 1000);
        eq.alertLevel = static_cast<int>(eq.magnitude - 2.0); // Rough mapping
        eq.alertLevel = qBound(0, eq.alertLevel, 4);
    ...
    void toggleGrid()
    {
        ...
        m_mapWidget->setShowGrid(m_showGrid);
    }
...
private:
    void setupUI()
    {
        ...
        auto *addBtn = new QPushButton("Add Random Earthquake");
        auto *clearBtn = new QPushButton("Clear All");
        auto *gridBtn = new QPushButton("Toggle Grid");
        ...
        connect(addBtn, &QPushButton::clicked, this, &EarthquakeMonitorWindow::addRandomEarthquake);
        connect(clearBtn, &QPushButton::clicked, this, &EarthquakeMonitorWindow::clearEarthquakes);
        connect(gridBtn, &QPushButton::clicked, this, &EarthquakeMonitorWindow::toggleGrid);
    }
    ...
    void loadSampleData()
    {
        // Load some sample earthquake data
        QVector<EarthquakeData> sampleEarthquakes = {
            {35.6762, 139.6503, 7.2, 32.0, QDateTime::currentDateTime().addSecs(-3600), "Tokyo, Japan", 3},
            {-33.8688, 151.2093, 5.5, 10.0, QDateTime::currentDateTime().addSecs(-1800), "Sydney, Australia", 2},
            {37.7749, -122.4194, 6.1, 8.0, QDateTime::currentDateTime().addSecs(-900), "San Francisco, USA", 2},
            {40.7128, -74.0060, 4.2, 15.0, QDateTime::currentDateTime().addSecs(-450), "New York, USA", 1},
            {-22.9068, -43.1729, 5.8, 25.0, QDateTime::currentDateTime().addSecs(-200), "Rio de Janeiro, Brazil", 2}
        };
        
        for (const auto &eq : sampleEarthquakes) {
            m_mapWidget->addEarthquake(eq);
        }
        
        // Center map on a good viewing location
        m_mapWidget->setMapCenter(20.0, 0.0);
        m_mapWidget->setZoomLevel(0.8);
    }
...
int main(int argc, char *argv[])
{
    ...
    EarthquakeMonitorWindow window;
    window.resize(1200, 800);
    window.setWindowTitle("Earthquake Alert System");
    window.show();
    
    return app.exec();
}

#include "main.moc" // For Q_OBJECT in source file
*/